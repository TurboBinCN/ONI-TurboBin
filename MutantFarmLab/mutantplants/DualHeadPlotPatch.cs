...
namespace MutantFarmLab.patches
{
    ...
    //        }catch (Exception ex)
    //        {
    //            PUtil.LogWarning($"[双头株] UpdateFertilization 操作异常: {ex}");
    //            return false;
    //        }
    //        return true;
    //    }
    //}
    //[HarmonyPatch(typeof(IrrigationMonitor.Instance), "UpdateIrrigation", new Type[] { typeof(float) })]
    //public class IrrigationMonitor_Instance_UpdateIrrigationPatch
    //{
    //    public static bool Prefix(IrrigationMonitor.Instance __instance, float dt)
    //    {
    //        try
    //        {
    //            if (__instance.gameObject == null)
