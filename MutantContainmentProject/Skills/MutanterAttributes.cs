...
namespace MutantContainmentProject.Skills
{
    public class MutanterAttributes
    {
        ...
        {
            var attribute = __instance.Add(new Attribute(AttributeMentalResistanceID, is_trainable: true, Attribute.Display.Skill, is_profession: true, 0f, null, null, "icon_errand_metal_resistance"));

            attribute.SetFormatter(new StandardAttributeFormatter(GameUtil.UnitClass.SimpleInteger, GameUtil.TimeSlice.None));
        }
        public static void AttributeSuccessRate(Database.Attributes __instance)
        {
            var attribute = __instance.Add(new Attribute(AttributeSuccessRateID, is_trainable: true, Attribute.Display.Skill, is_profession: true, 0f, null, null, "mod_strength"));

            attribute.SetFormatter(new StandardAttributeFormatter(GameUtil.UnitClass.SimpleInteger, GameUtil.TimeSlice.None));
        }
        public static void AttributeWorkingSpeed(Database.Attributes __instance)
        {
            var attribute = __instance.Add(new Attribute(AttributeWorkingSpeedID, is_trainable: true, Attribute.Display.Skill, is_profession: true, 0f, null, null, "mod_athletics"));
        ...
        {
            var attribute = __instance.Add(new Attribute(AttributeAttackDamageID, is_trainable: true, Attribute.Display.Skill, is_profession: true, 0f, null, null, "mod_stamina"));
